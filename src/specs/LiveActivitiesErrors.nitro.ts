...
export interface LiveActivitySystemError extends LiveActivityErrorInfo {
  code: LiveActivityErrorCode.NETWORK_ERROR | LiveActivityErrorCode.UNKNOWN_ERROR;
}

// All possible Live Activity errors
export type LiveActivityError = LiveActivityAuthorizationError | LiveActivitySystemError;
...
export namespace LiveActivityErrorFactory {
  ...
  export function createErrorFromNativeError(
    ...
    if (typeof nativeError === "object" && nativeError !== null) {
      ...
      const code = String(error.code || error.localizedDescription || "unknownError");
      const message = String(error.localizedDescription || "An unknown error occurred");
      const failureReason = error.failureReason ? String(error.failureReason) : undefined;
      ...
      const errorDomain = error.errorDomain ? String(error.errorDomain) : undefined;
      ...
      return createSystemError(
        mappedCode as LiveActivityErrorCode.NETWORK_ERROR | LiveActivityErrorCode.UNKNOWN_ERROR,
        message,
        {
          failureReason,
          recoverySuggestion,
    ...
    return createSystemError(LiveActivityErrorCode.UNKNOWN_ERROR, "An unknown error occurred", {
      nativeError,
      activityId,
    });
  }
...
export const LiveActivityErrorMessages = {
  ...
  [LiveActivityErrorCode.DENIED]: "A person deactivated Live Activities in Settings.",
  [LiveActivityErrorCode.GLOBAL_MAXIMUM_EXCEEDED]:
  ...
  [LiveActivityErrorCode.PERSISTENCE_FAILURE]: "The system couldn't persist the Live Activity.",
  [LiveActivityErrorCode.RECONNECT_NOT_PERMITTED]:
...
export const LiveActivityRecoverySuggestions = {
  ...
  [LiveActivityErrorCode.MISSING_PROCESS_IDENTIFIER]: "Restart the app and try again.",
  [LiveActivityErrorCode.PERSISTENCE_FAILURE]:
  ...
  [LiveActivityErrorCode.UNENTITLED]: "Ensure your app has the required Live Activity entitlement.",
  [LiveActivityErrorCode.UNSUPPORTED]: "Live Activities require iOS 16.2 or later.",
  [LiveActivityErrorCode.UNSUPPORTED_TARGET]:
  ...
  [LiveActivityErrorCode.NETWORK_ERROR]: "Check your internet connection and try again.",
...
export function isLiveActivityError(error: unknown): error is LiveActivityError {
  return (
    typeof error === "object" &&
    error !== null &&
    "code" in error &&
    "message" in error &&
    "timestamp" in error &&
    Object.values(LiveActivityErrorCode).includes((error as { code: LiveActivityErrorCode }).code)
  );
}
...
export function isSystemError(error: LiveActivityError): error is LiveActivitySystemError {
  return [LiveActivityErrorCode.NETWORK_ERROR, LiveActivityErrorCode.UNKNOWN_ERROR].includes(
    error.code,
  );
}
...
export const LiveActivityErrorSeverityMap = {
  [LiveActivityErrorCode.ATTRIBUTES_TOO_LARGE]: LiveActivityErrorSeverity.MEDIUM,
  [LiveActivityErrorCode.DENIED]: LiveActivityErrorSeverity.HIGH,
  [LiveActivityErrorCode.GLOBAL_MAXIMUM_EXCEEDED]: LiveActivityErrorSeverity.MEDIUM,
  [LiveActivityErrorCode.MALFORMED_ACTIVITY_IDENTIFIER]: LiveActivityErrorSeverity.HIGH,
  [LiveActivityErrorCode.MISSING_PROCESS_IDENTIFIER]: LiveActivityErrorSeverity.CRITICAL,
  [LiveActivityErrorCode.PERSISTENCE_FAILURE]: LiveActivityErrorSeverity.HIGH,
  [LiveActivityErrorCode.RECONNECT_NOT_PERMITTED]: LiveActivityErrorSeverity.HIGH,
  [LiveActivityErrorCode.TARGET_MAXIMUM_EXCEEDED]: LiveActivityErrorSeverity.MEDIUM,
  ...
  [LiveActivityErrorCode.UNSUPPORTED_TARGET]: LiveActivityErrorSeverity.CRITICAL,
...
export function getErrorSeverity(error: LiveActivityError): LiveActivityErrorSeverity {
