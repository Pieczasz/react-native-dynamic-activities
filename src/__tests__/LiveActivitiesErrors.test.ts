...
describe('Live Activities Error Handling', () => {
  describe('Error Codes', () => {
    it('should have Apple ActivityAuthorizationError codes matching exactly', () => {
      expect(LiveActivityErrorCode.ATTRIBUTES_TOO_LARGE).toBe(
        'attributesTooLarge'
      )
      expect(LiveActivityErrorCode.DENIED).toBe('denied')
      expect(LiveActivityErrorCode.GLOBAL_MAXIMUM_EXCEEDED).toBe(
        'globalMaximumExceeded'
      )
      expect(LiveActivityErrorCode.MALFORMED_ACTIVITY_IDENTIFIER).toBe(
        'malformedActivityIdentifier'
      )
      expect(LiveActivityErrorCode.MISSING_PROCESS_IDENTIFIER).toBe(
        'missingProcessIdentifier'
      )
      expect(LiveActivityErrorCode.PERSISTENCE_FAILURE).toBe(
        'persistenceFailure'
      )
      expect(LiveActivityErrorCode.RECONNECT_NOT_PERMITTED).toBe(
        'reconnectNotPermitted'
      )
      expect(LiveActivityErrorCode.TARGET_MAXIMUM_EXCEEDED).toBe(
  ...
  describe('Error Messages', () => {
    it('should provide correct error messages matching Apple descriptions', () => {
      expect(
        LiveActivityErrorMessages[LiveActivityErrorCode.ATTRIBUTES_TOO_LARGE]
      ).toBe(
      ...
      expect(
        LiveActivityErrorMessages[LiveActivityErrorCode.GLOBAL_MAXIMUM_EXCEEDED]
      ).toBe(
      ...
      ).toBe('The provided activity identifier is malformed.')
      expect(
        LiveActivityErrorMessages[
      ...
      expect(
        LiveActivityErrorMessages[LiveActivityErrorCode.PERSISTENCE_FAILURE]
      ).toBe("The system couldn't persist the Live Activity.")
      expect(
        LiveActivityErrorMessages[LiveActivityErrorCode.RECONNECT_NOT_PERMITTED]
      ).toBe(
        'The process that tried to recreate the Live Activity is not the process that originally created the Live Activity.'
      )
      expect(
        LiveActivityErrorMessages[LiveActivityErrorCode.TARGET_MAXIMUM_EXCEEDED]
      ).toBe(
      ...
      expect(
        LiveActivityErrorMessages[LiveActivityErrorCode.UNSUPPORTED_TARGET]
      ).toBe(
      ...
      expect(
        LiveActivityErrorMessages[LiveActivityErrorCode.NETWORK_ERROR]
      ).toBe('A network error occurred while processing the request.')
      expect(
        LiveActivityErrorMessages[LiveActivityErrorCode.UNKNOWN_ERROR]
  ...
  describe('Recovery Suggestions', () => {
    it('should provide helpful recovery suggestions', () => {
      ...
      ).toBe('Reduce the size of the Live Activity attributes to under 4KB.')
      expect(
        LiveActivityRecoverySuggestions[LiveActivityErrorCode.DENIED]
      ).toBe(
        'Ask the user to enable Live Activities in Settings > [Your App] > Live Activities.'
      )
      expect(
        LiveActivityRecoverySuggestions[LiveActivityErrorCode.UNENTITLED]
  ...
  describe('Error Factory Functions', () => {
    describe('createAuthorizationError', () => {
      ...
      it('should use default message when none provided', () => {
        ...
        expect(error.message).toBe(
          LiveActivityErrorMessages[LiveActivityErrorCode.DENIED]
        )
      })
    ...
    describe('createErrorFromNativeError', () => {
      ...
      it('should handle unknown native error codes', () => {
        ...
        const error =
          LiveActivityErrorFactory.createErrorFromNativeError(nativeError)
      ...
      it('should handle invalid native error input', () => {
        const error =
          LiveActivityErrorFactory.createErrorFromNativeError('invalid')
  ...
  describe('Error Severity', () => {
    it('should have correct severity mappings', () => {
      ...
      expect(
        LiveActivityErrorSeverityMap[LiveActivityErrorCode.UNENTITLED]
      ).toBe(LiveActivityErrorSeverity.CRITICAL)
      expect(
        LiveActivityErrorSeverityMap[LiveActivityErrorCode.UNSUPPORTED]
      ).toBe(LiveActivityErrorSeverity.CRITICAL)
      expect(
        LiveActivityErrorSeverityMap[LiveActivityErrorCode.NETWORK_ERROR]
      ).toBe(LiveActivityErrorSeverity.LOW)
      expect(
        LiveActivityErrorSeverityMap[LiveActivityErrorCode.ATTRIBUTES_TOO_LARGE]
      ).toBe(LiveActivityErrorSeverity.MEDIUM)
    })
    ...
    it('should return correct severity for errors', () => {
      ...
      expect(getErrorSeverity(criticalError)).toBe(
  ...
  describe('Error Domain', () => {
    it('should have correct error domain', () => {
      expect(LIVE_ACTIVITY_ERROR_DOMAIN).toBe(
        'com.dynamicactivities.liveactivities'
      )
    })
  })
